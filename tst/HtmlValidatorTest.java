...
public class HtmlValidatorTest {
     ...
     */
    private static String expectedOutputToString() {
        StringBuilder sb = new StringBuilder();
        try {
            Scanner fileScanner = new Scanner(new File("tst/test0_expected_output.txt"));
            while (fileScanner.hasNextLine()) {
                sb.append(fileScanner.nextLine()+ System.lineSeparator());
            }
        } catch (FileNotFoundException ex) {
            Assert.fail("tst/test0_expected_output.txt" + "not found. Make sure this file exists. Use relative path to root in front of the file name");
        }
        return sb.toString();
    }
